...
s_A = randint(0, 2**ea)
K_A = P2 + s_A * Q2
phi_A = E.isogeny(kernel=K_A, algorithm="factored")
...
s_B = randint(0, 3**eb)
K_B = P3 + s_B * Q3
phi_B = E.isogeny(kernel=K_B, algorithm="factored")
...
Shared_A = (
    Bob_pub[0].isogeny(kernel=K_BA, algorithm="factored").codomain().j_invariant()
)

K_AB = Alice_pub[1] + Bob_sec * Alice_pub[2]
Shared_B = (
    Alice_pub[0].isogeny(kernel=K_AB, algorithm="factored").codomain().j_invariant()
)
