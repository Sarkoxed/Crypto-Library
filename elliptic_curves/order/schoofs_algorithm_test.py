...
def test_get_xy(n=1):
    from sage.all import random_prime, GF, EllipticCurve
    for _ in range(n):
        p = random_prime(2**16)
        a = randint(0, p)
        b = randint(0, p)
...
def test_j_exists():
    ...
    y1_num, y1_den = get_y1_div_y(Q(x), Q(x**3 + a * x + b), x1_num, x1_den, cache1, p, -1)
...
def errors():
    ...
    print("order: ", e.order())
    print()
    print("remainders: ", [t % l for l in base])
    print()
