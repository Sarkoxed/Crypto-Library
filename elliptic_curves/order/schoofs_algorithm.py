...
from sage.all import GF, PolynomialRing, Primes, crt, gcd, prod, randint, var
...
def get_x1(x, y_squared, cache, q, ql):
    ...
    lambda_2_num = (
        y_squared
        * (y_squared ** ((q**2 - 1) // 2) * y_ql_den - y_ql_num) ** 2
        * x_ql_den**2
    )
    lambda_2_den = y_ql_den**2 * (x_q2 * x_ql_den - x_ql_num) ** 2

    x1_num = (
        lambda_2_num * x_ql_den
        - x_ql_num * lambda_2_den
        - x_q2 * lambda_2_den * x_ql_den
    )
        ...
        for j in range(1, (l - 1) // 2 + 1):
            ...
            if x1_x_j == 0:
                ...
                y1_num, y1_den = get_y1_div_y(
                    Q(x), Q(y_squared), x1_num, x1_den, cache, q, ql
                )
