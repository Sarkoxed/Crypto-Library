...
def attack(sequence, flag=True):
    ...
    if solver.check() == z3.sat:
        ...
        if flag:
            with open("inp.py", "at") as f:
                f.write(f"{state0, state1 =}")
        ...
        return next_sequence, state0, state1
...
if __name__ == "__main__":
    ...
    for i in range(0, len(sequence), 64):
        if len(sequence[i: i + 64]) < 64:
            break
        seq.extend(sequence[i: i + 64][::-1])
    next, st0, st1 = attack(seq[::-1])
    print("ned: ", next)

    from murmurhash import state_to_seed
    seed = state_to_seed(st0, st1)
    print(seed % 2**32)
