...
def GaussianExpected(base):
    a = pow(gamma(1 + len(base) / 2) * abs(Matrix(base).det()), 1 / len(base)) / sqrt(
        pi
    )
...
def LLL_check(base, delta=3 / 4, eta=1 / 2):
    ...
    mu = [
        [base[i].dot_product(w[j]) / w[j].norm() ** 2 for j in range(len(base))]
...
def reduce(k, l, mu, base):
    if (
        abs(mu[k][l]) <= 1 / 2
    ):  # i dunno, in algo its <= 1/2 but it doesn't equal sage LLL output(however it does better)
        return
    m = nearest(mu[k][l])
    base[k] -= m * base[l]
    mu[k][l] -= m
